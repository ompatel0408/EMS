try {
	document.addEventListener("DOMContentLoaded", () => {
		...
		xhr.open('POST', 'http://192.168.1.9:8080/EMS/EMSDirectorsDashboardServlet', true);
		xhr.onload = function() {
			if (xhr.status == 200) {
	...
	function getListSideBar(userId) {
		...
		var gradeData;
		var xhr = new XMLHttpRequest();
		xhr.open('GET', `http://192.168.1.9:8080/EMS/ShowManagementAccess?change=2&userId=${userId}`, true);
		...
		xhr.onload = function() {
			if (xhr.status == 200) {
				...
				} if (gradeData.dispatch == 1) {
					document.getElementById('dispatchItem-id').style.display = 'block';
